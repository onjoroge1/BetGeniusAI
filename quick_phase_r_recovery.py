"""
Quick Phase R Recovery - Simplified without LightGBM
...
class QuickPhaseRRecovery:
    """Quick Phase R recovery with essential guardrails"""
    ...
    def get_training_data(self, euro_only: bool = False):
        ...
        try:
            ...
            # Create features with enforced order
    ...
    def calculate_baselines(self, y_true: List[str]) -> Dict:
        ...
        market_adj = market_raw / market_raw.sum()
    ...
    def train_baseline_models(self, X_train: pd.DataFrame, y_train: List[str]) -> Dict:
        ...
        gb_model.fit(X_train, y_numeric)
        models['gradient_boosting'] = gb_model
        
        return models
    
    def evaluate_model(self, model, X_test: pd.DataFrame, y_test: List[str]) -> Dict:
        """Evaluate model and return metrics"""
        
        # Get predictions
        probs = model.predict_proba(X_test)
        
        # Ensure probabilities sum to 1 and are finite (guardrail)
        ...
        # Calculate metrics
        return self.get_baseline_metrics(y_test, probs)
    
    def run_quick_phase_r(self):
        """Run quick Phase R evaluation"""
        
        print("PHASE R - QUICK RECOVERY EVALUATION")
        print("=" * 60)
        
        # Load data
        df, X = self.get_training_data(euro_only=True)
        if df is None:
            return None
        ...
        # Check guardrails
        guardrails = self.check_guardrails(baselines, model_results)
        
        # Per-league breakdown
        league_results = {}
        for league_id, league_name in self.euro_leagues.items():
            league_mask = df['league_id'].iloc[split_idx:] == league_id
            if league_mask.sum() < 20:
                continue
            
            league_y = [y_test[i] for i in range(len(y_test)) if league_mask.iloc[i]]
            ...
            league_models = {}
            for model_name, model in models.items():
                league_metrics = self.evaluate_model(model, league_X, league_y)
    ...
    def check_guardrails(self, baselines: Dict, models: Dict) -> Dict:
        ...
        for model_name, metrics in models.items():
            ...
            # Check if significantly better than uniform
            uniform_ll = baselines['uniform']['logloss']
            improvement = (uniform_ll - model_ll) / uniform_ll * 100
            guardrails[f"{model_name}_beats_uniform_5pct"] = improvement > 5.0
    ...
    def generate_quick_report(self, results: Dict) -> str:
        """Generate quick Phase R recovery report"""
        
        lines = [
            "PHASE R - QUICK RECOVERY EVALUATION REPORT",
            "=" * 70,
        ...
        if best_model_ll < best_baseline_ll:
            ...
            lines.append("Models still worse than baselines - continue Phase R")
        
        # Top-2 accuracy status
        best_top2 = max(m['top2_accuracy'] for m in models.values())
        if best_top2 > 0.90:
            lines.append(f"Top-2 accuracy fixed: {best_top2:.1%} (target >90%)")
        else:
            lines.append(f"Top-2 accuracy needs work: {best_top2:.1%} (target >90%)")
        
        return "\n".join(lines)

def main():
    """Run quick Phase R recovery"""
    
    recovery = QuickPhaseRRecovery()
    
    # Run evaluation
    results = recovery.run_quick_phase_r()
    
    if results:
        # Generate report
        report = recovery.generate_quick_report(results)
        print("\n" + report)
        
        # Save results
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        
        with open(f'quick_phase_r_results_{timestamp}.json', 'w') as f:
            json.dump(results, f, indent=2, default=str)
        
        with open(f'quick_phase_r_report_{timestamp}.txt', 'w') as f:
            f.write(report)
        
        print(f"\nQuick Phase R recovery evaluation complete!")
        print(f"Results: quick_phase_r_results_{timestamp}.json")
        print(f"Report: quick_phase_r_report_{timestamp}.txt")
        
        # Check if ready for Phase A
        if results.get('guardrails_passed', {}).get('phase_r_ready', False):
            print("\nREADY FOR PHASE A - ACCURACY LIFT!")
        else:
            print("\nCONTINUE PHASE R - Guardrails not yet passed")

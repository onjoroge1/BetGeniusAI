...
- **CRITICAL: Odds Pipeline Fixed (Nov 8)**: Diagnosed and fixed broken market features causing 45% accuracy. Root cause: T-1h odds unavailable (0 matches), V2FeatureBuilder zero-filled to 0.33/0.33/0.34, model ignored constant features. Fix: Use "best available pre-kickoff" (cutoff=kickoff, n_books>=3), validate probabilities, raise exception to drop matches without odds. Coverage: 42.7% (3,764/8,809 matches). Expected new accuracy: 52-55% with active market features.

...
- **CLV Club Phase 1**: PRODUCTION-READY (September 29, 2025) - Advanced CLV detection system with professional-grade pricing inefficiency alerts. Features include: (1) De-juiced odds normalization removing bookmaker margins, (2) Trimmed mean consensus (10% tail removal) for robust market baseline, (3) Line stability metrics (≥70% threshold) ensuring reliable signals, (4) Desk group deduplication via YAML config preventing duplicate desk exposure, (5) Multi-tier gating (≥8 books for major leagues, ≥6 for minor, ≥2.0% CLV threshold), (6) Alert producer running every 60 seconds via scheduler, (7) TTL-based alerts with window tags (T-48to24, T-24to12, etc.), (8) Three REST APIs: /clv/club/opportunities (active alerts), /clv/club/alerts/history (paginated history), /clv/club/realized (Phase 2 stub). Database: bookmakers (with desk_group), clv_alerts (UUID primary keys with expiry), clv_realized (for future closing line capture).

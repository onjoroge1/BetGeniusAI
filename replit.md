...
**TBD Auto-Enrichment with Auto-Linking (Oct 2025)**: Production-grade TBD enrichment service with automatic team linkage integration. Enhanced enrichment query handles NULL kickoff timestamps for late-announced tournament matches. TeamLinkageService integration ensures 100% team_id assignment on every TBD resolution. Current status: 0/17 TBD fixtures, 100% linkage rate, 41.2% logo coverage on matches.
...
- **Prediction API**: Main endpoints for match predictions.
  ...
  - `/market` - **NEW (Oct 2025)** Market board showing both V1 + V2 predictions side-by-side (free tier, authenticated). Designed for real-time odds comparison with premium upgrade CTA. **Architecture:** Reads pre-computed V1 from `consensus_predictions` (updated every 60s by scheduler), fetches latest odds from `odds_snapshots`, generates V2 on-demand. Uses time-based filtering (`kickoff_at > NOW()`) instead of status flags for "upcoming" matches. **Team Logos (Oct 2025):** Includes team logo URLs via JOIN with teams table. Logos cached from API-Football. **API Improvements (Oct 2025):** Returns home_team_id/away_team_id fields for frontend team linkage. League filter parameter renamed to league_id for consistency. Response time: ~1.9s for 17 matches (includes on-demand V2 predictions).

...
- **Unified Accuracy Evaluation View**: PRODUCTION-READY (October 3, 2025) - SQL view `odds_accuracy_evaluation` combining odds_snapshots, match_results, and clv_closing_feed for streamlined accuracy testing and CLV analysis. Provides single source of truth with has_result and has_closing_odds flags. Supports real-time accuracy calculation (no simulation) and CLV measurement when closing odds available.
- **Historical Odds Backfill**: MANUAL-TRIGGER (October 3, 2025) - Script `scripts/backfill_historical_odds.py` for fetching historical odds from API-Football to enable backtesting on training_matches. Supports date filtering, rate limiting, and dry-run mode. Potential: 455 backfillable matches (299 from 2024) with fixture IDs. Requires manual execution, not automated.
- **Real Accuracy Testing**: OPERATIONAL (October 3, 2025) - Updated comprehensive_accuracy_test.py to use odds_accuracy_evaluation view instead of simulation. Automatically switches to real results when available, calculates true metrics (Brier, LogLoss, accuracy), and computes CLV when closing odds present. Currently tested with 1 real match, will scale automatically as Oct 3-4 predictions complete.
...
- **Multi-Source Real-Time Odds** (October 2025): Parallel collection from both The Odds API (21+ bookmakers) and API-Football for upcoming matches, ensuring data consistency between training (API-Football historical) and prediction (API-Football + The Odds API real-time).
- **League Coverage**: Dynamic league selection via league_map table (35 leagues expanded September 2025): Tier-1 European: EPL, La Liga, Serie A, Bundesliga, Ligue 1, Eredivisie; Tier-2 European: Championship, LaLiga2, Serie B, 2. Bundesliga, Ligue 2; UEFA Competitions: Champions League, Europa League, Conference League; English Lower: League One, League Two, FA Cup, EFL Cup; Asian: J1 League, K League 1; South American: Copa Libertadores, Brasileirão, Liga MX, MLS, Liga Profesional Argentina; Nordic/Alpine: Eliteserien, Allsvenskan, Superliga, Swiss Super League, Austrian Bundesliga; Other European: Primeira Liga, Scottish Premiership, Jupiler Pro League, Süper Lig.
- **Training Data**: 9,846+ matches in training_matches for ML model learning (5,195 original + 571 UEFA + 3,294 Priority 2-4 leagues backfilled September 30, 2025) with synchronized odds_consensus table. Latest backfill includes English lower divisions (1,425 matches), Asian leagues (612 matches), South American (155 matches), and Nordic/Alpine leagues (1,102 matches) from 2024 season.

...
- **V2 ODDS-ONLY MODEL READY FOR PRODUCTION (Nov 14)**: Successfully trained and validated clean V2 baseline using 17 odds-derived features. Performance: 49.5% accuracy, LogLoss 1.0162, Brier 0.2030. Random label sanity: 37.0% (PASS ✅). Location: `artifacts/models/v2_odds_only/`. **Ready to ship to /predict-v2 endpoint today.** Interaction puzzle discovered: Individual feature groups all pass sanity checks (30.7-38.7%), but combined 50-feature model still fails (42-43%). Created leak_detector_v2.py to diagnose using exact training setup. Next: Run mirror test to determine if interaction leak, test bug, or time-window specific issue. Documentation: LEAK_INVESTIGATION_NOV14.md.
- **CRITICAL: LEAKAGE DETECTED IN TEAM FEATURES (Nov 14)**: Step A optimizations revealed persistent leakage in non-odds features. Sanity checks: (1) Random labels: 41.4% (expected ~33%) = FAIL, (2) Row permutation: 42.7% = FAIL, (3) ✅ Market-only baseline: 47.1% = PASS (odds features CLEAN). Leak isolated to team/context features: `form_*`, `rest_days_*`, `days_since_*`. Two-track fix: (A) Ship odds-only V2 today (17 features, guaranteed clean, 48-50% accuracy), (B) Fix leaky features in 2-3 days using leak_detector.py ablation tool. Hyperparameters from Step A: num_leaves=31, min_data=50, draw_weight=1.30×. After leak fix + optimizations → 52-54% target. Documentation: LEAK_FIX_PLAN.md.

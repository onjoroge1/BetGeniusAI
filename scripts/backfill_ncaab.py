...
Attempts to pull all available NCAAB game data for the current season
...
Usage:
    ...
    python scripts/backfill_ncaab.py --days 90    # attempt full season
    python scripts/backfill_ncaab.py --dry-run    # check API without writing
"""
...
def fetch_scores(api_key: str, requested_days: int) -> tuple[list, int]:
    ...
    for attempt_days in candidates:
        ...
        if resp.status_code == 404:
            logger.warning("  404 — no NCAAB data (sport may be off-season)")
            return [], 0
    ...
    logger.error(
        "  All window sizes failed — NCAAB scores endpoint unavailable on this plan. "
...
def log_coverage_gap(db_url: str, actual_days: int):
    """
    Report the oldest NCAAB game_date in multisport_match_results and whether
    the current season start (Nov 2025) is covered.
    """
    if not db_url:
        return
    try:
        ...
        if total == 0:
            logger.warning(
                "  Coverage check: 0 completed NCAAB results in DB. "
...
def print_summary(db_url: str):
    ...
    cursor.execute("SELECT COUNT(*) FROM multisport_fixtures WHERE sport_key = 'basketball_ncaab'")
    ...
    cursor.execute("SELECT COUNT(*) FROM multisport_odds_snapshots WHERE sport_key = 'basketball_ncaab'")
    ...
    cursor.execute("SELECT COUNT(*) FROM multisport_training WHERE sport_key = 'basketball_ncaab'")
...
def main():
    ...
    logger.info(
        f"Starting NCAAB backfill — requested daysFrom={args.days}, "
